...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous).view(-1, 1)
        x_batch = additional_context['x_batch']
        z_batch = additional_context['z_batch']
        chunk_descriptor = additional_context['chunk_descriptor']
        context_mask = additional_context['context_mask']

        if cond is None:
            output = model(x, x_batch, z_batch, chunk_descriptor, t_input, context_mask, **model_kwargs)
        else:
    ...
    def model_fn(x, t_continuous):
        if guidance_type == "uncond":
            ...
            t_input = get_model_input_time(t_continuous).expand(x.shape[0])
